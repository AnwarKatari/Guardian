...
export default function HomePage({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  ...
  return (
    <div className="w-full space-y-8 pb-20 relative font-sans text-neutral-900">
      ...
      {/* Safety Header */}
      <section className="flex justify-between items-center relative z-10 pt-4 border-b border-neutral-100 pb-6">
      ...
      {/* Grid Layout: Bento Columns on Large Screens, Vertical Stack on Mobile */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        
        {/* Left Column: Core Status Indicators (Active state, Advisor, Alert Feeds) */}
        <div className="lg:col-span-5 space-y-8">
          ...
          <motion.section 
            ...
            className="p-8 bg-[#0d0d0d] text-white rounded-[40px] space-y-6 relative overflow-hidden shadow-2xl border border-white/5 group hover:border-blue-500/30 transition-all block"
          >
            ...
            <div className="relative z-10 space-y-5">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/5">
                ...
                <motion.div 
                   whileHover={{ x: 3 }}
                   className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] text-blue-500 italic cursor-pointer"
                >
        ...
        {/* Right Column: Interaction Cards, Academy Banner & Action Grids */}
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4 relative z-10">
            <motion.div 
              ...
              className="p-6 bg-white border border-neutral-100 rounded-[32px] shadow-sm space-y-4 hover:border-blue-500/30 transition-all hover:bg-blue-50/50"
            >
          ...
          {/* Safety Academy & Gamification Banner */}
          <motion.section
            initial={{ y: 15, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.15 }}
            onClick={() => setActiveTab('academy')}
            className="relative p-8 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-[32px] shadow-xl shadow-blue-500/10 hover:shadow-2xl hover:shadow-blue-500/20 active:scale-95 cursor-pointer transition-all overflow-hidden group z-10 block"
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(255,255,255,0.15)_0%,_transparent_70%)]" />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 text-white/10 group-hover:scale-110 transition-transform duration-500">
              <Zap size={100} className="fill-current" />
            </div>
            <div className="flex items-center justify-between relative z-10">
              <div className="space-y-1 flex-1">
                <span className="text-[8px] font-mono font-black text-amber-300 tracking-[0.3em] uppercase">LEVEL {Math.floor((profile?.points || 0) / 200) + 1} PROTÉGÉ</span>
                <h4 className="text-xl font-display font-black tracking-tight uppercase italic leading-none flex items-center gap-1.5">
                  Academy & Rewards Center
                </h4>
                <p className="text-[10px] text-blue-100 font-bold uppercase tracking-wider">
                  Earn points, unlock {profile?.badges?.length || 0} badges & friendly challenges
                </p>
              </div>
              <div className="flex flex-col items-end shrink-0 pl-6 border-l border-white/10 ml-4">
                <span className="text-3xl font-mono font-black text-amber-300 leading-none">
                  {profile?.points || 0}
                </span>
                <span className="text-[7px] font-black text-blue-100 uppercase tracking-widest mt-1">PTS</span>
              </div>
            </div>
          </motion.section>
